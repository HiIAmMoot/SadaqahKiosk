...
- [ ] **E7. Pull the power within 60s of an update install, then boot: the watchdog re-arms and a healthy build is not rolled back.** logcat `UpdateWatchdog` shows `Armed for +120000ms` after boot, and the new version is still running five minutes later. Repeat with a build that crashes on launch: it must be rolled back after the re-armed window. Install through the app's own updater, not `adb install`: only that path saves the backup APK and arms the watchdog. Without a usable backup, logcat shows `No backup APK to roll back to` and nothing rolls back. If the tablet's clock resets on power loss, run it with network time off as well; that is the case the fix exists for. *(PR #22)*
...
- [x] `[auto]` **P1. A provisioned device exports the configuration it was given.** Round trip: provision from a known payload, export from the device, compare. A raw file comparison fails on a correct provisioning because salt, iv and ciphertext are regenerated on every export. The check compares decoded settings in both directions against a declared set of device-scoped fields. **It does not verify the credentials themselves** — only that a key was restored, not that it is the right one. A truncated key, or an affiliate key and a telemetry key swapped, would pass. That is accepted because the affiliate key is identical across the fleet; if that changes, this is the first gap to close.
