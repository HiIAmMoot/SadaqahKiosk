...
class MainActivity : FragmentActivity() {
     ...
     * night’s update maintenance and telemetry flush. It also stops a kiosk
     * that logs in repeatedly from accumulating one nightly coroutine per login,
     * which the previous fire-and-forget version did.
     *
     * **Why it loops.** The old version was armed once and re-armed only from
     ...
     * offline at 02:00 therefore lost nightly reinit, auto-update maintenance
     * *and* the telemetry flush permanently, until an operator logged in by
     * hand. Being offline at 02:00 now simply means trying again tomorrow.
     *
    ...
    fun scheduleDailyLoginReset(affiliateKey: String) {
        ...
        dailyMaintenanceJob = lifecycleScope.launch {
            while (true) {
                ...
                // nowhere else, so this must sit outside the guards above — a
                // reinit or update-maintenance failure must not also cost the
                // nightly flush. flushTelemetry carries its own guard.
